...
export class MilestonePostgresRepository implements MilestoneRepository {
  ...
  async addMilestone(milestoneDto: CreateMilestoneDTO): Promise<Milestone> {
    const milestoneEntity = this.repository.create({
      ...
      startDate: milestoneDto.startDate
        ? new Date(milestoneDto.startDate)
        : null,
  ...
  async update(milestoneDto: UpdateMilestoneDTO): Promise<Milestone> {
    const milestoneEntity = await this.repository.findOneBy({
      milestoneID: milestoneDto.milestoneID,
    });

    if (!milestoneEntity) {
      throw new Error(
        `Milestone with id: ${milestoneDto.milestoneID} was not found`,
      );
    }

    milestoneEntity.name = milestoneDto.name ?? milestoneEntity.name;
    milestoneEntity.description =
      milestoneDto.description ?? milestoneEntity.description;
    milestoneEntity.parentId =
      milestoneDto.parentId ?? milestoneEntity.parentId;
    milestoneEntity.startDate = milestoneDto.startDate
      ? new Date(milestoneDto.startDate)
      : milestoneEntity.startDate;
    milestoneEntity.endDate = milestoneDto.endDate
      ? new Date(milestoneDto.endDate)
      : milestoneEntity.endDate;
    ...
    return new Milestone(
      ...
      updatedMilestoneEntity.startDate
        ? updatedMilestoneEntity.startDate.toISOString()
        : null,
      updatedMilestoneEntity.endDate
        ? updatedMilestoneEntity.endDate.toISOString()
        : null,
